...
public class AIWanderingTraderEntity extends WanderingTraderEntity {
    ...
    private void assignRandomPersonality() {
        double gc = AIVillagerMod.AIVillagerConfig.config.generousTraderChance;
        double mc = AIVillagerMod.AIVillagerConfig.config.mysteriousTraderChance;
    ...
    private void initializeTrades() {
        ...
        switch (personality) {
            case "generous" -> TraderTradeGenerator.generateGenerousTrades(o, random, this);
            case "mysterious" -> TraderTradeGenerator.generateMysteriousTrades(o, random, this);
            case "shrewd" -> TraderTradeGenerator.generateShrewdTrades(o, random, this);
        }
        TraderTradeGenerator.addEnchantedItemTrades(o, random, this);
    }
    ...
    private void handleGift(PlayerEntity player, ItemStack gift) {
        ...
        if (trustLevel > ("generous".equals(personality) ? 50 : 70) && tradesRemaining > 0) {
            TraderTradeGenerator.addSpecialTrade(getOffers(), random, this);
            tradesRemaining--;
    ...
    public void afterUsing(TradeOfferList offer) {
        ...
        if (random.nextFloat() < ("generous".equals(personality) ? 0.4f : 0.2f) && trustLevel > 50)
            TraderTradeGenerator.refreshTrades(getOffers(), random, this);
    }
