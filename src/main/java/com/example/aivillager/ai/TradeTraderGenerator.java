...
public class TraderTradeGenerator {
    ...
    private static ItemStack ancBook(Random r){ItemStack b=new ItemStack(Items.ENCHANTED_BOOK);Enchantment[][] pairs={{Enchantments.PROTECTION,4},{Enchantments.BLAST_PROTECTION,4},{Enchantments.SHARPNESS,5},{Enchantments.SMITE,5},{Enchantments.SILK_TOUCH,1},{Enchantments.FORTUNE,3},{Enchantments.INFINITY,1},{Enchantments.MENDING,1}};List<Enchantment[]> list=Arrays.asList(pairs);Collections.shuffle(list,r);for(int i=0;i<Math.min(r.nextInt(3)+3,list.size());i++)EnchantedBookItem.addEnchantment(b,new EnchantmentLevelEntry(list.get(i)[0],list.get(i)[1].intValue()));return b;}
